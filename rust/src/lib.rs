...
mod _native {
    ...
    fn merge_points_removing_overlap<'py>(
        py: Python<'py>,
        connected_crop_rows: PyReadonlyArray2<f64>,
        points_in_rows: PyReadonlyArray2<f64>,
        row_information: PyReadonlyArray2<f64>,
        tolerance: f64,
        num_threads: usize
    ) -> Bound<'py, PyArray2<f64>> {
        let c = connected_crop_rows.as_array().to_owned();
        let p = points_in_rows.as_array().to_owned();
        let r = row_information.as_array().to_owned();
        // Call the Rust function
        let result = crate::rust_fn::multithreaded_merging(&c, &p, &r, tolerance, num_threads);
        result.into_pyarray(py)
    }
}

mod rust_fn {
    use numpy::ndarray::{ViewRepr, OwnedRepr, Dim, ArrayBase, Axis, Array1, Array2, s};
    use rayon::prelude::*;
    ...
    pub fn determine_points_relations(
        ...
        */
        ...
        let r1 = row1.slice(s![1..]).to_owned().into_shape_with_order((3, 2)).unwrap();
        let r2 = row2.slice(s![1..]).to_owned().into_shape_with_order((3, 2)).unwrap();

        let mut max_dist = 0.0;
        let mut pair = (0, 0);
        for (i, a) in r1.axis_iter(Axis(0)).enumerate() {
            for (j, b) in r2.axis_iter(Axis(0)).enumerate() {
        ...
        let row_1_far: ArrayBase<OwnedRepr<f64>, Dim<[usize; 1]>> = r1.row(pair.0).to_owned();
    ...
    fn merge_points_removing_overlap(
        points_in_rows: &ArrayBase<OwnedRepr<f64>, Dim<[usize; 2]>>,
        row_information: &ArrayBase<OwnedRepr<f64>, Dim<[usize; 2]>>,
        tolerance: f64,
        row_segment: ArrayBase<ViewRepr<&f64>, Dim<[usize; 1]>>
    ) -> Vec<ArrayBase<OwnedRepr<f64>, Dim<[usize; 1]>>> {
        /*
