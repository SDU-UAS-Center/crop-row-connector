"""Combine crop rows from connections."""

import numpy as np
import pandas as pd

from crop_row_connector.find_connection_of_rows_between_two_tiles import Tile


class CombineCropRowsFromConnections:
    """Combine crop rows from connections."""
    ...
    def connect_crop_rows_of_2_tiles(self, tile_1_number: int, tile_2_number: int, connections: np.ndarray) -> None:
        """
        ...
        for row in connections:
            ...
            if index_row_1.size > 0 and index_row_2.size > 0:
                if self.connected_crop_rows[index_row_1[0]][0] != self.connected_crop_rows[index_row_2[0]][0]:
                    ...
                    # print("row: ", row)
                    self.connect_two_existing_full_crop_rows(
    ...
    def connect_crop_row_to_existing_crop_row(
        self,
        new_tile_number: int,
        new_row: np.ndarray,
        existing_tile_number: int,
        ...
        self.connected_crop_rows = np.vstack(
            (
                ...
                np.array(
                    [
                        self.connected_crop_rows[index[0]][0],
                        new_tile_number,
                        new_row,
                        existing_tile_number,
                        existing_row,
                        None,
                        None,
                        None,
    ...
    def add_connection(self, tile_number: int, row: np.ndarray, index: np.ndarray) -> None:
        if self.connected_crop_rows[index[0], 5] is None:
            ...
            print("A crop row tried to connect to more than 4 other crop rows, this is not possible")
            # assert False, "A crop row tried to connect to more than 4 other crop rows, this is not possible"

    def connect_two_existing_full_crop_rows(
        self,
        index_row_1: np.ndarray,
        tile_number_1: int,
        row_1: np.ndarray,
        index_row_2: np.ndarray,
        tile_number_2: int,
        row_2: np.ndarray,
    ) -> None:
    ...
    def move_row_and_adjust_indexes(self, smallest_row_index: np.ndarray, largest_row_index: np.ndarray) -> None:
