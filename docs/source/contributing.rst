...
Thank you for your interest in contributing to *Crop Row Connector* and we welcome all pull request. To get set for development on *Crop Row Connector* see the following.
...
A changelog is automatically generated when making a release based on commit messages using git cliff. For this to work commits needs follow the `Conventional Commits <https://www.conventionalcommits.org/en/v1.0.0/>`_ standard. This is enforced with pre-commits. See `examples <https://www.conventionalcommits.org/en/v1.0.0/#examples>`_ for inspiration. The changelog will sort the commits into the following types:
...
* Change version number in :code:`pyproject.toml` and commit to main.
